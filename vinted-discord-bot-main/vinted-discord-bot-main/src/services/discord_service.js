...
export async function createPrivateThread(
  ...
  if (!thread_channel_id) {
      throw new Error('Thread channel is not configured.');
  }
...
export async function checkVintedChannelInactivity(client) {
    try {
        if (!guild_id) {
            Logger.warn('Guild ID is not configured. Skipping channel inactivity checks.');
            return;
        }

        const guild = client.guilds.cache.get(guild_id);
        if (!guild) {
            Logger.warn('Discord guild is not available yet. Skipping channel inactivity checks.');
            return;
        }
        ...
        for (const channel of channels) {
            ...
            // Check if the lastUpdated is more than configured hours ago
            if (hoursAgo(lastUpdated) > discordConfig.channel_inactivity_hours) {
                ...
                if (!member) {
                    Logger.info('User is no longer in the server, deleting the channel');
