  ...
  return (
    <DashboardLayout>
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <section className="overflow-hidden rounded-[32px] border border-white/10 bg-[#111116] p-6 shadow-[0_0_0_1px_rgba(255,255,255,0.03)] sm:p-8">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.28em] text-violet-300">Vinted</p>
            ...
            <div className="flex flex-wrap gap-3">
              <button onClick={syncNow} disabled={syncing} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-white px-4 py-3 text-sm font-semibold text-zinc-950 transition hover:bg-violet-100 disabled:opacity-60">
        ...
        {loading ? (
          <div className="mt-6 grid gap-4 md:grid-cols-3">
            {[...Array(3)].map((_, index) => <div key={index} className="h-24 animate-pulse rounded-[24px] border border-white/10 bg-[#111116]" />)}
          </div>
        ) : !stats ? (
          <div className="mt-6 rounded-[28px] border border-dashed border-white/10 bg-[#111116] p-8 text-center text-sm text-zinc-400">Aucun compte Vinted lié pour le moment. Connectez votre compte depuis les paramètres pour démarrer.</div>
        ) : (
          <>
            <div className="mt-6 grid gap-4 md:grid-cols-3">
              <StatCard label="Chiffre d’affaires total" value={`€${Math.round(stats.totalRevenue || 0)}`} />
